...
import { ChangeDetectionStrategy, Component, ElementRef, HostListener, OnDestroy, Renderer2, inject, input } from '@angular/core'
  ...
  constructor() {
    const subscriber = toObservable(this.config).subscribe((nodeData) => {
      if (nodeData) {
        ...
        // TODO: you'll have to setup something to make sure the string format is correct
        this.renderer.setStyle(this.hostElementRef.nativeElement, 'background-image', nodeData.img)
